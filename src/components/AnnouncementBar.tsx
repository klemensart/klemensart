...
export default function AnnouncementBar({ page }: { page: string }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        // Dismiss edilmemiş olanları filtrele
    ...
    return () => {
      if (didSet.current) setBarOffset(0);
    };
  }, [page]);
      ...
      .then(({ data }) => {
        ...
        for (const a of data) {
          if (a.link_url && a.badge_text) {
            // /atolyeler/sinema-klubu → sinema-klubu
