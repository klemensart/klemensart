...
export default function MuzeRehberBanner() {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 left-4 sm:left-auto sm:bottom-6 sm:right-6 z-50 w-auto sm:w-[380px] rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ease-out ${
      ...
      <div className="relative p-6">
        {status === "idle" || status === "error" ? (
          ...
          <div className="text-center py-2">
            <button
              onClick={() => { setVisible(false); setTimeout(() => setMounted(false), 350); }}
              className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-full hover:bg-black/5 transition-colors text-gray-500 hover:text-gray-800"
              aria-label="Kapat"
            >
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
            ...
            <h3 className="text-lg font-bold text-[#2D2926] mb-1">Rehberiniz Hazır!</h3>
            <p className="text-sm text-gray-600 mb-4">PDF e-postanıza gönderildi. Hemen indirmek için:</p>
            <a
              href={pdfUrl}
              target="_blank"
