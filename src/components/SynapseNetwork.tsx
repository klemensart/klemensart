...
export default function SynapseNetwork() {
  return (
    <div
      className="absolute inset-y-0 right-0 w-1/2 pointer-events-none select-none z-10"
      aria-hidden="true"
    >
        ...
        {connections.map((c, i) => {
          ...
          return (
            <motion.path
              ...
              transition={{ duration: c.dur, repeat: Infinity, ease: "easeInOut", delay: c.delay, times: OPACITY_T }}
            />
          );
        })}

        {nodes.map((n) => {
          const isHub = n.size === "lg";
          return (
            <motion.g
              key={n.label}
              animate={{ y: FLOAT_Y, x: FLOAT_X }}
              transition={{ duration: n.floatDur, repeat: Infinity, ease: "easeInOut", delay: n.floatDelay, times: FLOAT_T }}
            >
              {isHub && (
                <motion.circle
                  cx={n.x} cy={n.y} r={6}
                  fill="none" stroke="#FF6D60" strokeWidth={0.75}
                  animate={{ r: [6, 26], opacity: [0.45, 0] }}
                  transition={{ duration: 4.8, repeat: Infinity, ease: "easeOut", delay: 1.5 }}
                />
              )}
              <motion.circle
                cx={n.x} cy={n.y} r={isHub ? 4.5 : 3}
                fill="#FF6D60" filter="url(#dot-glow)"
                animate={{
                  opacity: [0.25, 0.72, 0.30, 0.60, 0.25],
                  r: isHub ? [4, 5.5, 4, 5, 4] : [2.5, 4, 2.8, 3.5, 2.5],
                }}
                transition={{ duration: n.floatDur * 0.9, repeat: Infinity, ease: "easeInOut", delay: n.floatDelay + 0.4, times: OPACITY_T }}
              />
              <motion.text
                x={n.x} y={n.y + n.labelDy} textAnchor={n.ta}
                style={{
                  fontFamily: "var(--font-playfair), 'Playfair Display', Georgia, serif",
                  fontStyle: "italic", fontSize: isHub ? 18 : 13.5,
                  fill: "#2C2319", letterSpacing: isHub ? "0.03em" : "0.01em",
                }}
                animate={{ opacity: [0.30, 0.62, 0.34, 0.55, 0.30] }}
                transition={{ duration: n.floatDur, repeat: Infinity, ease: "easeInOut", delay: n.floatDelay + 0.65, times: OPACITY_T }}
              >
                {n.label}
              </motion.text>
            </motion.g>
