...
const ARTICLE_ID = "684f4b4f-79c9-4307-90e4-4635649b9bd3";
...
async function main() {
  // 1. Kapak görselini Supabase'e yükle
  const imgBuffer = fs.readFileSync("/tmp/oscar-cover3.jpg");
  const imgPath = "article-covers/altin-heykelcigin-oteki-yuzu.jpg";

  console.log("Kapak görseli yükleniyor...");
  const { error: upErr } = await supabase.storage
    .from("email-assets")
    .upload(imgPath, imgBuffer, { contentType: "image/jpeg", upsert: true });

  if (upErr) {
    console.error("Görsel yükleme hatası:", upErr.message);
    return;
  }

  const { data: pubUrl } = supabase.storage.from("email-assets").getPublicUrl(imgPath);
  const imageUrl = pubUrl.publicUrl;
  console.log("Görsel URL:", imageUrl);

  // 2. Makaleyi güncelle (italic content + kapak görseli)
  ...
  console.log("- Film isimleri italic yapıldı");
  console.log("- Yönetmen isimleri italic yapıldı");
  console.log("- Kapak görseli eklendi");
}
